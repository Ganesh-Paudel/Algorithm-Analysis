...
from analysis.results_aggregator import AlgorithmResult


def recordPerformance(size, result: Union[Dict[str, Any], AlgorithmResult]):
    ...
    if isinstance(result, AlgorithmResult):
        ...
        data = {
            ...
            "StaticRuntimeSec": [result.static_runtime],
            "DynamicRuntimeSec": [result.dynamic_runtime],
            "StaticCost": [result.static_cost],
            "CostAfterTraffic": [result.cost_after_traffic],
            "ReroutedCost": [result.rerouted_cost],
            "Improvement": [result.improvement],
            "ImprovementPercent": [result.improvement_percent],
            "NodesExplored": [result.nodes_explored],
            "StaticMemoryMB": [result.static_memory],
            "DynamicMemoryMB": [result.dynamic_memory],
        }
    else:
